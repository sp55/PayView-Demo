...
// 中心符号圆点的大小
CGFloat const kInputViewSymbolWH = 20;
...
- (void)addNotification{
    ...
    [[NSNotificationCenter defaultCenter] addObserverForName:UITextFieldTextDidChangeNotification object:nil queue:nil usingBlock:^(NSNotification * note) {
        ...
        if (length == self.places && self.InputViewDidCompletion) {
            self.InputViewDidCompletion(_textField.text);
        }
...
- (void)setupContents:(NSInteger)pages{
    ...
    for (int i = 0; i < pages; i++) {
        ...
        UIBezierPath *path = [UIBezierPath bezierPathWithOvalInRect:CGRectMake(0, 0, kInputViewSymbolWH, kInputViewSymbolWH)];
...
- (void)layoutSubviews{
    ...
    CGFloat margin = kInputViewSymbolWH * 0.5;
